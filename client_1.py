...
class PrivateGui:
    def __init__(self, target: str, me: str, me_nickname, gui: tk.Toplevel):
        self.is_show = False
        self.target = target
        self.me = me
...
class clientGui:
    ...
    def createPrivateWindow(self, other):
        def build_window():
            ...
            private_window.title(self.username + "<->" + other)
            private_window.geometry("580x380")
            return private_window

        if other in self.private_windows:
            if not self.private_windows[other].is_show:
                self.private_windows[other].gui = build_window()
                self.private_windows[other].showGui()
            else:
                self.private_windows[other].gui.lift()

        else:

            private_gui = PrivateGui(other, self.username, self.nickname, build_window())
